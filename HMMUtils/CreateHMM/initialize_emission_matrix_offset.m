function emission_matrix = initialize_emission_matrix_offset(d, k, n, mean_range, std_range)
    ...
    for var = 1:n
        ...
        % Ensure means are spaced apart systematically
        mean_vals = linspace(mean_range(1), mean_range(2), k);
        
        for state = 1:k
            % Randomly perturb the mean slightly
            mean_val = mean_vals(state) + 0.05 * d * (rand() - 0.5);
